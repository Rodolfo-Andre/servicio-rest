...
public class ComandaController {
   ...
   * try {
   * CheckStatus statusPlatos =
   * verificarSiExistePlatos(baseData.getListaPlatos());
   * Mesa mesa = mesaService.obtenerPorId(baseData.getNumeroMesa());
   * 
   * if (statusPlatos.getStatus().equals(CheckStatus.StatusError)) {
   * return ResponseEntity.ok()
   * .body(Map.of("mensaje", statusPlatos.getMensaje(), "status",
   * statusPlatos.getStatus()));
   * }
   * 
   * if (mesa.getEstado().equals("Libre")) {
   ...
   * String status = "error";
   * return ResponseEntity.ok().body(Map.of("mensaje", mensaje, "status",
   * status));
   * }
   * }
   */
   ...
   * registrarNuevaComanda(BaseDataInput baseData) {
   * Mesa mesa = mesaService.obtenerPorId(baseData.getNumeroMesa());
   ...
   * 
   * Empleado empleado =
   * empleadoService.findEmpleadoByIdUsario(baseData.idUsuario);
  ...
  /*
   * private ResponseEntity<Map<String, String>>
   * actualizarComandaExistente(BaseDataInput baseData) {
   ...
   * CheckStatus checkStatus = new CheckStatus(CheckStatus.StatusSuccess,
   ...
   * public ResponseEntity<Map<String, String>> eliminarComanda(@RequestBody
   * DeleteDetalleComandaInput input) {
   * String mensaje = "";
   * String status = "success";
   * try {
   * DetalleComanda detalleComanda =
   * detalleComandaService.findDetalleComandaByPlatoIdAndComandaId(input.platoID,
   * input.comandaID);
